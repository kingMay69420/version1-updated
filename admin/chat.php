...
/** Build partner list: admin ↔ coordinators; coordinator ↔ admins (works either way) */
if (isAdmin()) {
...
/** Active partner (?with=ID), default to first available */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send']) && $with) {
    ...
    if ($body === '') {
        ...
        header("Location: /admin/chat.php?with=" . $with);
        exit();
    }
}
    ...
    <?php else: ?>
      <?php foreach ($partners as $p): ?>
        ...
        <a class="chat-person <?= ($with === (int)$p['id']) ? 'active' : '' ?>"
           href="/admin/chat.php?with=<?= (int)$p['id'] ?>">
  ...
  <section class="chat-panel">
    ...
    <?php else: ?>
      <div class="chat-header">
        ...
        <form method="get" action="/admin/chat.php" class="chat-refresh">
